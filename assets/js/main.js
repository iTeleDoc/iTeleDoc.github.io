	...
	// Main.
		...
		// Methods.
			...
			$main._hide = function(addState) {
				...
				// AUTO-CLOSE SEARCH BARS WHEN CLOSING ARTICLE
				const searchBox = document.getElementById('searchBox');
				const searchInput = document.getElementById('searchInput');
				const searchIconWrapper = document.getElementById('searchIconWrapper');
				if (searchBox) searchBox.classList.remove('active');
				if (searchInput) searchInput.value = "";
				if (searchIconWrapper) searchIconWrapper.className = "icon solid fa-search";
				...
				if (dSearchBox) dSearchBox.classList.remove('active');
				if (dSearchInput) dSearchInput.value = "";
				if (dIconWrapper) dIconWrapper.className = "icon solid fa-search";

				if (typeof filterProtocols === 'function') filterProtocols();
				if (typeof filterDiseases === 'function') filterDiseases();
...
function toggleSearch(event) {
    ...
    if (iconWrapper && (iconWrapper.classList.contains('fa-globe') || iconWrapper.classList.contains('fa-cloud-download-alt'))) {
        searchInput.blur(); // Dismiss mobile keyboard on submit
...
async function executeExternalV3ProtocolLookup(queryText) {
    ...
    dynamicCardDeck.innerHTML = `
        <div class="approach-card" style="width: 100%; grid-column: 1 / -1; display: flex; flex-direction: column; align-items: start; position: relative;">
            ...
            <ul style="margin: 0 0 1.5rem 1.25rem; padding: 0; color: rgba(255,255,255,0.75); font-size: 0.9rem; line-height: 1.6; list-style-type: disc !important;">
...
// Click-away interaction controller to clear/collapse search bars when clicking outside them
document.addEventListener('click', function(event) {
    const searchBox = document.getElementById('searchBox');
    const searchInput = document.getElementById('searchInput');
    const iconWrapper = document.getElementById('searchIconWrapper');
    ...
    if (dSearchBox && !dSearchBox.contains(event.target)) {
        if (dSearchBox.classList.contains('active')) {
...
/* ==========================================================================
   OFFICIAL GOOGLE GEMINI CLINICAL LOOKUP API
   ========================================================================== */
async function fetchHighPrecisionData(queryString) {
    // REPLACE THIS WITH YOUR OFFICIAL GOOGLE GEMINI API KEY FROM GOOGLE AI STUDIO
    const apiKey = "AIzaSyCwN2xr4Dwi5ZmU6JbDQDXm3lCeesutN4Y"; 
    const url = `https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${apiKey}`;

    const systemInstruction = `You are a clinical medicine assistant. Provide structured, evidence-based data for the requested pathology or treatment. Return your answer precisely as a JSON object with two keys: "tag" (a short category string, e.g., "Cardiology") and "steps" (an array of exactly 4 concise, clear clinical actions, parameters, or guidelines). Do not include any extra markdown formatting or backticks like \`\`\`json outside of the valid JSON object string.`;
